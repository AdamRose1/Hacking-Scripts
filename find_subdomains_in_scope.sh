...
# SHODAN
# The purpose of the awk command below is used to filter out rows that shodan returned that do not contain any domain name. This is achieved by looking at the first column in each returned row, if the first column has any letter besides 'a' or 'A' then it saves the row to the file (since the first column in a shodan returned row that has no domain name will be either the A or AAAA). The second purpose of the awk command is used to print out the full domain found (otherwise it only prints the found subdomain which is not helpful if multiple targets are used in this shodan command since we won't know which domain is associated to the found domain). The purpose of the sed command below is to remove the first line returned for each target shodan runs before piping it to the awk command. This is important to do because shodan outputs the targeted domain name on the first line. Without removing that first line, it would return $target.$target. For exmaple: shodan domain example.com would return example.com on the first line. Then awk would return it as example.com.example.com which is not correct. If want also IPv6 then add to shodan -T AAAA
for target in $(cat in_scope_target_list.txt);do shodan domain -T A,CNAME $target| sed '1d'| awk -v target=$(echo $target) '$1 ~ /[^aA]/ {print $1=$1"."target}' | sort -uf >> find_subdomains_in_scope/shodan_output;done 
